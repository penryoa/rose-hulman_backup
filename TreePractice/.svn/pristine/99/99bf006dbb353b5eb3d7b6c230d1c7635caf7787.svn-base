...
public class BinarySearchTree<T extends Comparable<? super T>> {
	 ...
	 * integers, so you may use a cast.
	 * 
	...
	public String numChildrenOfEachNode() {
		return root.numChildrenEachNode();
	}
	...
	public class BinaryNode {
		...
		public boolean zigZag(boolean wentLeft, boolean started) {
			if (this == NULL_NODE) {
				return true;
			...
			if (this.left != NULL_NODE && this.right == NULL_NODE) {
				if (started) {
					return this.left.zigZag(true, false);
				} else if (wentLeft) {
					return false;
				} else {
					return this.left.zigZag(true, false);
				...
				if (started) {
					return this.right.zigZag(false, false);
				} else if (!wentLeft) {
					return false;
				} else {
					return this.right.zigZag(false, false);
		...
		public String numChildrenEachNode() {
			if (this == NULL_NODE) {
				return "";
			} else if (this.left == NULL_NODE && this.right == NULL_NODE) {
				return "0";
			} else if (this.left != NULL_NODE && this.right != NULL_NODE) {
				return "2" + this.left.numChildrenEachNode() + this.right.numChildrenEachNode();
			} else if (this.left != NULL_NODE && this.right == NULL_NODE) {
				return "1" + this.left.numChildrenEachNode();
			} else {
				return "1" + this.right.numChildrenEachNode();
			}
		}
		...
		public int getDepth(T item, int depth) {
			...
			if ((int) this.element < (int) item) {
				return right.getDepth(item, depth + 1);
			} else if ((int) this.element > (int) item) {
				return left.getDepth(item, depth + 1);
			}
		...
		public int countPositives() {
			if (this == NULL_NODE) {
				...
				return left.countPositives() + right.countPositives();
			}
