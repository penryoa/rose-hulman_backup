...
def run_test_largest_number():
    ...
    expected = 123456
    answer = largest_number(([12,34,56], [1,2,3,4,5,6], [123456]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    #     if seq_seq[k]!=[]:
    #         temp_largest = seq_seq[k][0]
    #         for j in range(len(seq_seq[k])):
    #             if seq_seq[k][j] > temp_largest:
    #                 temp_largest = seq_seq[k][j]
    #         seq = seq + [temp_largest]
    #
    # largest = None
    #
    ...
    # Construct a sequence of the largest number of each sequence in seq_seq.
    seq = []
    for k in range(len(seq_seq)):
        if len(seq_seq[k]) == 1:
            seq = seq + seq_seq[k]
        if len(seq_seq[k]) >= 2:
            for j in range(len(seq_seq[k])-1):
                if seq_seq[k][j] > seq_seq[k][j+1]:
                    seq = seq + [seq_seq[k][j]]


    # Of those numbers, we're going to find the largest. If there is no number, the function returns None.
    largest = None
    for i in range(len(seq)):
        if len(seq) == 1:
            largest = seq[0]
...
def run_test_largest_negative_number():
    ...
    expected = -1
    answer = largest_negative_number(([-3,-5,9], [-1,2,3,4,5,-6], [-123456]))
    print('Expected and actual are:', expected, answer)

    expected = -200
    answer = largest_negative_number(([12,34,56], [1,2,3,4,-200,6], [123456]))
    print('Expected and actual are:', expected, answer)

    expected = -12
    answer = largest_negative_number(([-12,-34,-56], [1,2,3,4,5,6], [-123456]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    # Construct a sequence of the negative numbers in each sequence in seq_seq called neg.
    #   Within neg, the largest number is added to seq.
    seq = []
    neg = []
    for k in range(len(seq_seq)):
        if len(seq_seq[k]) == 1:
        ...
        if len(seq_seq[k]) >= 2:
            ...
            if len(neg) >=2 :
                temp_largest = neg[0]
                for f in range(len(neg)-1):
                    if neg[f+1] > temp_largest:
                        temp_largest = neg[f+1]
                seq = seq + [temp_largest]
    ...
    # WHEW. I feel like there was probably an easier way to do that, but this works at least!
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        for j in range(len(seq_seq)-1):
            for h in range(len(seq_seq[j+1])):
                if seq_seq[j+1][h] == seq_seq[0][k]:
                    return True
    return False
