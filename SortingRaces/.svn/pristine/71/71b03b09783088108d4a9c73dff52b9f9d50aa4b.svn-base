...
// that takes a class type as a parameter, you should use this test file instead of the other
// one.   

public class AllBinaryHeapGenericConstructorTest {
	...
	// }
	// str = str + "]";
	// assertEquals(str, heap1.toString());
	// assertPartiallyOrdered(heap1);
	// BinaryHeap<Integer> heap2 = new BinaryHeap<Integer>();
	// for (int i = 1000; i > 0; i--) {
	// heap2.insert(i);
	// }
	// assertPartiallyOrdered(heap2);
	// }
	...
	// private void assertPartiallyOrdered(BinaryHeap<?> heap) {
	// int size = heap.size();
	// for (int level = 0; level < Math.log(size) / Math.log(2); level++) {
	// int startIndex = (int) Math.pow(2, level);
	// int endIndex = (int) (Math.pow(2, level + 1) - 1);
	// for (int i = startIndex; i <= endIndex; i++) {
	// int j = 2 * i;
	// // The nested signum calls make these equivalent to less than or
	// // equals as opposed to less than
	// if (j < size) {
	// assertEquals((int) Math.signum(2 * Math.signum(heap
	// .compare(i, j)) - 1), -1);
	// }
	// j = 2 * i + 1;
	// if (j < size) {
	...
	/**
	 * Randomly adds and then deletes characters Checks to see if the heap
	 * status is maintained
	 * 
	 */
	// @Test
	// public void testRandomDeleteMin() {
	// BinaryHeap<Character> heap = new BinaryHeap<Character>();
	// for (int i = 0; i < 500; i++) {
	// heap.insert((char) ((123 - 65) * Math.random() + 65));
	// }
	// for (int i = 0; i < 500 * Math.random(); i++) {
	//
	// heap.deleteMin();
	// }
	// assertPartiallyOrdered(heap);
	...
	/**
	 * This example was in the slides from class
	 * 
	 */
	// @Test
	...
	// heap.insert(32);
	// heap.insert(31);
	// assertEquals(13, (int) heap.deleteMin());
	// assertEquals("[null, 14, 19, 16, 26, 21, 19, 68, 65, 31, 32]",
	// heap.toString());
	// assertPartiallyOrdered(heap);
	// }
	...
	public void testRandomNumbersSort() {
		for (int j = 1; j < 100; j++) {
			...
			BinaryHeap<Integer> numberHeap = new BinaryHeap<Integer>(Integer.class);
	...
	public void testSort() {
		...
		BinaryHeap<Integer> intHeap = new BinaryHeap<Integer>(Integer.class);
		...
		String[] strArray = new String[] { "in", "the", "second", "step", "a",
				...
				"heap", "in", "step", "one" };
		BinaryHeap<String> strHeap = new BinaryHeap<String>(String.class);
	...
	public void testSort2() {
		...
		BinaryHeap<String> backwardsAlphaHeap = new BinaryHeap<String>(String.class);
		...
		BinaryHeap<String> randomAlphaHeap = new BinaryHeap<String>(String.class);
		randomAlphaHeap.sort(randomAlpha);
		assertEquals("[a, b, c, d, e, f]", Arrays.toString(randomAlpha));
		points += 1;

		String[] Alpha = new String[] { "a", "b", "c", "d", "e", "f" };
		BinaryHeap<String> AlphaHeap = new BinaryHeap<String>(String.class);
		...
		intHeap = new BinaryHeap<Integer>(Integer.class);
		intHeap.sort(integers);
		assertEquals("[1, 2, 3, 4, 5, 6]", Arrays.toString(integers));
