...
public class MapProblems {
	...
	public static boolean hasDuplicates(int[] input) {
		...
		for (int k=0; k < input.length; k++) {
			int value = input[k];
			visited.put(value, visited.getOrDefault(value, 0)+1);
		}
	...
	public static int computeScore(String hand) {
		...
		for(int k=0; k < hand.length(); k++) {
			char card = hand.charAt(k);
			visited.put(card, visited.getOrDefault(card,0) + 1);
			
			totalScore += visited.get(card);
		}
