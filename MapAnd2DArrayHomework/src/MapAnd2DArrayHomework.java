...
public class MapAnd2DArrayHomework {
	...
	public static char mostCommonCharacter(String input) {
		HashMap<Character, Integer> charAndInts = new HashMap<Character, Integer> ();
		char mostCommon = input.charAt(0);
		
		for (int k=0; k < input.length(); k++) {
			charAndInts.put(input.charAt(k), charAndInts.getOrDefault(input.charAt(k), 0) + 1);
		}
		
		
		for (int m=1; m < input.length(); m++) {
			if (charAndInts.get(mostCommon) < charAndInts.get(input.charAt(m))) {
				mostCommon = input.charAt(m);
			}
		}
	...
	public static HashMap<String,ArrayList<Integer>> reverseHashmap(HashMap<Integer,String> initialMap) {
		...
		HashMap<String,ArrayList<Integer>> reverse = new HashMap<String,ArrayList<Integer>>();
		
		for (int k=0; k < initialMap.size(); k++) {
			reverse.put(initialMap.get(k), reverse.getOrDefault(k, 1) + initialMap[k]);
	...
	public static String getTemperatureDropCity(int[] temps, String[] tempCity) {
		...
		HashMap<String, Integer> tempsAndCities = new HashMap<String, Integer> ();
		
		for (int k=0; k < tempCity.length; k++) {
			if (tempsAndCities.containsKey(tempCity[k]) ) {
				if (tempsAndCities.get(tempCity[k]) > temps[k]) {
					return tempCity[k];
				}
			} else {
				tempsAndCities.put(tempCity[k], temps[k]);
			}	
		}
		return null;
	}
	...
	public static int longestRepeatSequence(char[][] data) {
		...
		for (int m=0; m<myStr.length()-1; m++) {
			int tempCount = 0;
			for (int k=m; k < myStr.length(); k++) {
				if (myStr.charAt(m) == myStr.charAt(k)) {
					if (k == m) {
						tempCount += 1;	
					} else {
						if (myStr.charAt(k) == myStr.charAt(k-1)) {
							tempCount += 1;	
						}
					}
				}
