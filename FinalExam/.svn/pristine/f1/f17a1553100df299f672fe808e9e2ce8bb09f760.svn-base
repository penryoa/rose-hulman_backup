...
public class AdjacencyListGraph<T> extends Graph<T> {
	...
	public int numDirectedTriangles() {
		int count = 0;
		for (Vertex first : keyToVertex.values()) {
			// loop through all vertexes
			for (Vertex second : first.successors) {
				// loop through each successor of the vertex
				for (Vertex third : second.successors) {
					List<Vertex> thirdSuccessors = third.successors;
					if (thirdSuccessors.contains(first)) {
						count++;
					}
				}
			}
