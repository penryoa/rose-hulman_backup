...
/**
 * In this project, we start with a box on the screen which does not show up
 * properly unless the screen is resized.
 * 
 * You will need to add or modify classes to:
 * 
 * Stage 1 #1 Get the box to appear without resizing the window. #2 Replace the
 * button which advances one tick to use a Timer instead.
 * 
 * Stage 2 #3 Make a button to add more boxes to the simulation
 * 
 * Stage 3 #4 Make a button to toggle whether the boxes on the screen are moving
 * or not
 * 
 * Stage 4 #5 Make a button to freeze all boxes (color and movement) while
 * preserving their state to be unpaused on a subsequent press
 *
