...
public class HashMapProblem {
	...
	public static HashMap<Integer, ArrayList<Integer>> returnHashMapOfSumsAsKeys(ArrayList<ArrayList<Integer>> listOfNumberLists ) {
		...
		return helper(newMap, listOfNumberLists, 0);
	}
	
	public static HashMap<Integer, ArrayList<Integer>> helper(HashMap<Integer, ArrayList<Integer>> map, ArrayList<ArrayList<Integer>> list, int index) {
		
		if (index == list.size()) {
			return map;
		}
		
		int sum = arrayListSummation(list.get(index));
		
		if (sum != 0) {
			map.put(sum, list.get(index));
		}
		
		return helper(map, list, index + 1);
	}
