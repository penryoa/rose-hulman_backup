...
public class TeamGradebook {
	...
	private String handleAddStudent(String studentName) {
		...
		if (this.students.add(newStudent) == true) {
			return "ok";	
		} else {
			return null;
		}
	 ...
	 * every time add-absence is called
	 * 
	...
	private String handleAddAbsence(String studentName) {
		Student currentStudent = getStudentByName(studentName);
		currentStudent.absenceCount++;
		return "ok";
	}
	...
	private String handleGetAbsences(String studentName) {
		int numAbs = getStudentByName(studentName).absenceCount;
		String resultAsString = Integer.toString(numAbs);
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		for (int k=0; k < memberNames.size(); k++) {
			Student currentStudent = getStudentByName(memberNames.get(k));
			studentsInTeam.add(currentStudent);
		}
		Team newTeam = new Team(teamName, studentsInTeam);
	...
	public Student getStudentByName(String name) {
		if (this.students.contains(name)) {
			int studentIndex = this.students.indexOf(name);
	...
	private String handleAddGrade(String teamName, double grade) {
		int teamIndex = this.teams.indexOf(teamName);
		Team currentTeam = this.teams.get(teamIndex);
		
		currentTeam.addGrade(grade);
	...
	private String handleGetAverage(String studentName) {
		...
		return Double.toString(getStudentByName(studentName).getAverage());
	}
	...
	private String handleGetBestTeam() {
		...
		HashMap<String, Double> teamGrades = new HashMap<String, Double>();
		
		for(Team team:this.teams) {
			teamGrades.put(team.getName(), team.avgGrade());
		}
		
		String maxTeam = " ";
		double maxAvg = 0.0;
		
		for(String keys: teamGrades.keySet()) {
			if (teamGrades.get(keys) > maxAvg) {
				maxAvg = teamGrades.get(keys);
				maxTeam = keys;
			}
		}
